...
const AdminDashboard = () => {
  ...
  const fetchDbSessions = async () => {
    ...
    try {
      const response = await fetch(`${backendUrl}/monitor/db-sessions`, { credentials: "include" });
  ...
  const fetchDbStats = async () => {
    setIsMonitoringLoading(true);
    try {
      const response = await fetch(`${backendUrl}/monitor/db-stats`, { credentials: "include" });
      const data = await response.json();
      setDbStats(data);
    } catch (error) {
  ...
  const deleteUser = async (id) => {
    ...
    if (confirmed.isConfirmed) {
      try {
        await fetch(`${backendUrl}/api/admin/users/${id}`, { method: "DELETE", credentials: "include" });
  ...
  const updateUser = async () => {
    try {
      const response = await fetch(`${backendUrl}/api/admin/users/${editingUser.user_id}`, {
        method: "PUT",
