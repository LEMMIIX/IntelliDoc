...
# Available ONNX files in the repository
ONNX_FILES = [
    "model.onnx",
    "model_fp16.onnx",
...
def download_model():
    """Download complete model including base files and ONNX files"""
    ...
    try:
        ...
        snapshot_download(
            repo_id=MODEL_NAME,
            local_dir=BASE_PATH,
            token=os.environ.get('HF_TOKEN'),  # Use token from environment if available
            local_dir_use_symlinks=False,
            ignore_patterns=[".*"],
            force_download=True,
        )
        
        # Step 2: Download ONNX files directly from Hugging Face
